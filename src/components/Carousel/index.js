...
export default function Carousel({ children, refContainer }) {
  ...
  const threshold = 100;
  const itemShow = window.innerWidth < 767 ? 1 : 4;
  ...
  const fnCheckIndex = useCallback(
    (e) => {
      if (e.propertyName === "left") {
        ...
        if (index <= 0) {
          ...
          setIndex(0);
        } else if (index >= cardCount - itemShow) {
          refDragHandler.current.style.left = `${-(
            (cardCount - itemShow + isMobile) *
            cardSize
          )}px`;
          setIndex(cardCount - itemShow);
    ...
    [cardCount, cardSize, index, itemShow]
  );
